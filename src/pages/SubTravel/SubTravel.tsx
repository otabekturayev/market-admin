...
import { ModulsType, SubTravelType } from "../../types/types";
import EditSubTravel from "./modules/EditSubTravel";
import AddSubTravel from "./modules/AddSubTravel";
...
const SubTravel = () => {
  ...
  const [subTrivelIdeasSingleData, setSubTrivelIdeasSingleData] = useState<SubTravelType>();
  ...
  const { data, isLoading, refetch } = useFetch<SubTravelType>({
    key: ['subtravel', pagination.current, pagination.pageSize, search],
    url: '/subtravel',
    config: {
      params: {
  ...
  const handleEdit = (record: SubTravelType, type: ModulsType) => {
  ...
  const handleDelete = (record: SubTravelType) => {
    mutate({ id: record?.id });
  };

  const columns: ColumnsType<SubTravelType> = [
    {
      title: "№",
      render: (_, __, index) => (
        <span>
  ...
  return (
    <div>
      ...
      >
        {formType === "edit" ? <EditSubTravel onCancel={handleCancel} data={subTrivelIdeasSingleData} refetch={refetch}/> : <AddSubTravel refetch={refetch} onCancel={handleCancel}/>} 
      </UniversalModal>
    </div>
  );
};

export default SubTravel;
